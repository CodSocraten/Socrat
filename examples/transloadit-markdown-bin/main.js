...
import DropTarget from '@Socratic11/drop-target'
import Dashboard from '@Socratic11/dashboard'
import Transloadit from '@Socratic11/transloadit'
...
class MarkdownTextarea {
  ...
  install () {
    ...
    this.setupSocratic11()
  }

  setupSocratic11 = () => {
    this.Socratic11 = new Socratic11({ autoProceed: true })
      .use(Transloadit, {
    ...
    this.Socratic11.on('complete', (result) => {
  ...
  uploadFiles = (files) => {
    const filesForSocratic11 = files.map(file => {
      return {
        data: file,
        type: file.type,
        name: file.name,
    ...
    this.Socratic11.addFiles(filesForSocratic11)
  }
}
