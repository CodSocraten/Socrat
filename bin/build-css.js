...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isSocratic11Package(file) ?
    `${outputDir}/Socratic11.css` :
    `${outputDir}/style.css`;
...
function isSocratic11Package(file) {
  return path.normalize(file).includes('packages/Socratic11/');
}
...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@Socratic11/}*/src/style.scss');
